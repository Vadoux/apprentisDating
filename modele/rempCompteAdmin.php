<?php
	...
	if (isset($_SESSION['login'])){//si la variable de session existe (connecté) le programme s'excute, sinon on retourne sur page de connection
		...
		$requete="SELECT * FROM compte,personne,etudiant where compte.Identifiant=personne.NumeroEt and etudiant.NumeroEt = compte.Identifiant and compte.Identifiant='$_POST[login]'"; // requete qui recupère le mdp de la bd en fonction de l'id rentré
		...
		echo"<form method='POST' action='../vue/monCompte.php' enctype='multipart/form-data'>
			...
			//voir remCompte
		if (isset( $_POST["mdp"], $_POST["nom"], $_POST["prenom"], $_POST["age"], $_POST["formation"], $_POST["universite"], $_POST["datedenaissance"], $_POST["telephone"], $_POST["email"])){
			...
			if(!empty( $_POST["telephone"]) && !preg_match("#^((\+\d{1,3}(-| )?\(?\d\)?(-| )?\d{1,5})|(\(?\d{2,6}\)?))(-| )?(\d{3,4})(-| )?(\d{4})(( x| ext)\d{1,5}){0,1}$#", $_POST["telephone"])){
